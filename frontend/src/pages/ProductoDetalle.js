...
export default function ProductoDetalle() {
  ...
  // 3 medidas por defecto para escaleras, barras, etc.
  ...
  useEffect(() => {
    api.get(`/productos/${id}`)
      .then((res) => {
        ...
        setIndiceCarrusel(0);
        // Resetear a 3 medidas al cargar nuevo producto
        setMedidas([
          { largo: "", ancho: "" },
          { largo: "", ancho: "" },
          { largo: "", ancho: "" }
  ...
  // Obtener solo medidas válidas (con largo y ancho > 0)
  const obtenerMedidasValidas = () => {
    return medidas.filter(item => {
  ...
  if (producto?.tipoVenta === "metro_lineal" || producto?.tipoVenta === "metro_cuadrado") {
    ...
    if (modoCotizacion === "todas") {
      areaIngresada = medidasValidas.reduce((total, item) => {
        const largo = Number(item.largo) || 0;
        const ancho = Number(item.ancho) || 0;
      ...
      areaIngresada = (Number(medidaSeleccionada?.largo) || 0) * (Number(medidaSeleccionada?.ancho) || 0);
    }
  ...
  const generarPDF = async () => {
    try {
      ...
      detalleMedidas.forEach((item) => {
        ...
        } else if (producto.tipoVenta === "paquete" || producto.tipoVenta === "caja") {
          pdf.text(`Área a cubrir: ${item.area.toFixed(2)} m²`, 20, y);
        } else {
          pdf.text(
  ...
  const mostrarBotonesMedidas = () => {
    return producto?.tipoVenta === "metro_lineal" || producto?.tipoVenta === "metro_cuadrado" || 
           producto?.tipoVenta === "caja" || producto?.tipoVenta === "pieza";
  };
              ...
              <div className="cotizador-box" ref={cotizadorRef}>
                ...
                <div className="medidas-container">
                  {producto.tipoVenta === "metro_lineal" ? (
                    medidas.map((item, index) => (
                      <div key={index} className="medida-card">
                        ...
                        <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '12px' }}>
                          Ingresa el largo y ancho en metros para calcular el área (opcional).
                        </p>
                        {modoCotizacion === "una" && (
                    ...
                    // Para productos por caja, paquete, pieza
                    medidas.map((item, index) => {
                      const largo = Number(item.largo) || 0;
                      const ancho = Number(item.ancho) || 0;
                      const areaMedida = largo * ancho;
                ...
                {(areaIngresada > 0 || Number(medidas[0]?.area || 0) > 0) && (
                  <div className="resultado-cotizacion">
                    {producto.tipoVenta === "metro_lineal" ? (
                      ...
                      // PRODUCTOS POR CAJA, PAQUETE, PIEZA - CON DETALLES COMPLETOS
                      <>
                        <div className="resultado-grid">
                          <div className="resultado-item">
                          ...
                          {producto.tipoVenta === "caja" && (
                            <>
                              <div className="resultado-item">
                                <span className="resultado-label">📦 Piezas por caja</span>
                                <span className="resultado-valor">{producto.piezasCaja || '-'}</span>
                              </div>
                              <div className="resultado-item">
                                <span className="resultado-label">📐 Medida por pieza</span>
                              ...
                              {tieneCobertura && (
                                <div className="resultado-item">
                                  <span className="resultado-label">📐 Cobertura por pieza</span>
                                  <span className="resultado-valor">{(Number(producto.cobertura) || 0).toFixed(2)} m²</span>
                                </div>
                              )}
                              {coberturaPorUnidad > 0 && (
                                <div className="resultado-item">
                                  <span className="resultado-label">📦 Cobertura por caja</span>
                                  <span className="resultado-valor">{coberturaPorUnidad.toFixed(2)} m²</span>
                                </div>
                              )}
                            </>
                          ...
                          {producto.tipoVenta === "paquete" && (
                            <>
                              <div className="resultado-item">
                                <span className="resultado-label">📦 Piezas por paquete</span>
                                <span className="resultado-valor">{producto.piezasCaja || '-'}</span>
                              </div>
                              <div className="resultado-item">
                                <span className="resultado-label">📐 Medida por pieza</span>
                              ...
                              {coberturaPorUnidad > 0 && (
                                <div className="resultado-item">
                                  <span className="resultado-label">📦 Cobertura por paquete</span>
                                  <span className="resultado-valor">{coberturaPorUnidad.toFixed(2)} m²</span>
                                </div>
                              )}
                            </>
                          ...
                          <div className="resultado-item destacado">
                            ...
                            <span className="resultado-valor principal">{cantidadNecesaria} {producto.tipoVenta === "caja" ? "cajas" : producto.tipoVenta === "paquete" ? "paquetes" : "piezas"}</span>
                          </div>
                        </div>
                        ...
                        <div className="detalle-calculo">
                          ...
                          {producto.tipoVenta === "caja" && (
                            <>
                              ...
                              <p>Cobertura por caja: <strong>{coberturaPorUnidad.toFixed(2)} m²</strong></p>
                              <p>Cantidad necesaria: <strong>{cantidadNecesaria} cajas</strong></p>
                              <p style={{ color: '#16a34a', fontSize: '1.1rem', marginTop: '6px' }}>
                                💰 {cantidadNecesaria} cajas × ${Number(precioFinal).toLocaleString()} = <strong>${Number(total).toLocaleString()}</strong>
                              </p>
                            </>
                          )}
                          {producto.tipoVenta === "paquete" && (
                            <>
                              <p>Área a cubrir: <strong>{areaIngresada.toFixed(2)} m²</strong></p>
                              <p>Cobertura por paquete: <strong>{coberturaPorUnidad.toFixed(2)} m²</strong></p>
                              <p>Cantidad necesaria: <strong>{cantidadNecesaria} paquetes</strong></p>
                              <p style={{ color: '#16a34a', fontSize: '1.1rem', marginTop: '6px' }}>
                                💰 {cantidadNecesaria} paquetes × ${Number(precioFinal).toLocaleString()} = <strong>${Number(total).toLocaleString()}</strong>
                              </p>
                            </>
                          )}
                          {producto.tipoVenta === "pieza" && (
                            <>
                              <p>Área a cubrir: <strong>{areaIngresada.toFixed(2)} m²</strong></p>
                              <p>Cobertura por pieza: <strong>{coberturaPorUnidad.toFixed(2)} m²</strong></p>
                              <p>Cantidad necesaria: <strong>{cantidadNecesaria} piezas</strong></p>
                              <p style={{ color: '#16a34a', fontSize: '1.1rem', marginTop: '6px' }}>
                                💰 {cantidadNecesaria} piezas × ${Number(precioFinal).toLocaleString()} = <strong>${Number(total).toLocaleString()}</strong>
                              </p>
                            </>
                          )}
                        </div>
                        ...
                        <div className="nota-tipo-venta">
                          ℹ️ Este producto se vende por <strong>{producto.tipoVenta === "caja" ? "caja" : producto.tipoVenta === "paquete" ? "paquete" : "pieza"}</strong>
                          {producto.tipoVenta === "caja" && ` • ${producto.piezasCaja || 1} piezas por caja`}
                          {producto.tipoVenta === "paquete" && ` • ${producto.piezasCaja || 1} piezas por paquete`}
                        </div>
                      </>
                    )}
